...
QEMU's RISC-V CPU object exposes a ``big-endian`` boolean property which
models a big-endian-only hardware implementation, where the ``MBE``/``SBE``/
``UBE`` bits are hardwired to 1. When the property is set, the CPU is reset
with those bits initialised to 1, so the guest starts executing in big-endian
data mode from the reset vector. The property is a static, per-CPU hardware
configuration option and is not meant to be toggled at runtime. When the
Hypervisor extension is enabled, the virtual-supervisor endianness bits follow
the same fixed CPU configuration.

The property does not model a mixed-endian implementation where software can
toggle endianness bits at runtime. QEMU's RISC-V CPUs treat these fields as
fixed by the CPU configuration: they are reset to 0 by default and to 1 when
